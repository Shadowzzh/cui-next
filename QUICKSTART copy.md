...
# Should see:
# - CLAUDE.md
# - state-templates.json (NEW!)
# - skills/ (4 skills)
# - commands/ (5 commands)
# - hooks/ (hooks.json + 4 hook scripts)
```

### Step 2: Choose Your Testing Mode

The framework now supports **two modes** with dedicated commands:

#### 🎯 For HackTheBox/VulnHub/TryHackMe (CTF Mode)
```bash
/start-ctf 10.10.10.1
```

**What this does:**
- Automatically sets `scenario_type: "ctf"`
- Full attack chain: recon → exploit → privesc
- Goal: Capture user.txt + root.txt flags
- Stop hook blocks until both flags captured

#### 🔬 For Vulhub CVE Testing (Vulhub Mode)
```bash
/start-vulhub 127.0.0.1:8080
```

**What this does:**
- Automatically sets `scenario_type: "vulhub"`
- CVE auto-discovery: identify service → find CVE → exploit → verify
- Goal: Discover CVE and collect evidence
- Stop hook blocks until CVE identified and all evidence collected

**Reconnaissance process:**
1. Enumerate service and version on 127.0.0.1:8080
2. Search for CVE using searchsploit
...
```bash
/start-pentest 10.10.10.1  # Shows deprecation notice, uses CTF mode
```
