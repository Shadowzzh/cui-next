...
export function DiffViewer({ oldValue, newValue, language = 'javascript' }: DiffViewerProps) {
  ...
  const displayOldValue =
    !isExpanded && shouldShowExpandButton ? oldLines.slice(0, 8).join('\n') : oldValue;
  const displayNewValue =
    !isExpanded && shouldShowExpandButton ? newLines.slice(0, 8).join('\n') : newValue;
  ...
  const renderContent = (source: string): JSX.Element => {
    ...
    try {
      ...
      return (
        <span dangerouslySetInnerHTML={{ __html: highlighted }} style={{ display: 'inline' }} />
      );
    } catch (error) {
      // 如果高亮失败，返回原始文本
      return <span>{source}</span>;
