...
cui fully supports Claude Code's **custom commands** and **auto-activating skills**:

- **Commands** (`/command-name`): Reusable workflow templates that you can invoke explicitly
- **Skills**: Specialized behaviors that activate automatically based on conversation context

You can create custom commands and skills by adding Markdown files to `.claude/commands/` and `.claude/skills/` directories in your project or globally in `~/.claude/`.

📚 **[Read the complete Commands and Skills Guide](COMMANDS_AND_SKILLS.md)** for examples and best practices.
...
Having issues with installation or running the server? Check out the [Troubleshooting Guide](TROUBLESHOOTING.md) for solutions to common problems including:

- better-sqlite3 native module compilation failures
- workbox-precaching build failures
